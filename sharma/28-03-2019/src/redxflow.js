...
class Redxflow extends React.Component {
  ...
  componentWillReceiveProps({ starwardata }) {
    debugger;
    if (starwardata.results.length > 0) {
      debugger;
      this.setState({ myvalue: starwardata.results });
    }
  }
  ...
  render() {
    ...
    //Whenever you receives somethings from props you should get them directly
    //you need to use componentWillReciveProps
    // let a = this.props.starwardata.results;
    // this.setState({ myvalue: a });
    // console.log(this.state.myvalue);

    // window.alert(JSON.stringify(b));
    // window.alert(JSON.stringify(this.state.myvalue));
    //  {
    //  this.state.myvalue.results &&
    //  this.state.myvalue.results.map(o, i => {
    //  return <div>{o[i]}</div>;
    //});
    //}
      ...
      <div className="ant-table">
        ...
        <div className="App">
          ...
          <div>
            ...
            {this.state.myvalue &&
              this.state.myvalue.map(value => {
                return (
                  <div className="cusButton">
                    <a>{JSON.stringify(value.name)}</a>
                  </div>
                );
              })}
                    ...
                    {item !== 'key' && (
                      <Form.Item label={item}>
                        <Input
                          ...
                          onChange={event =>
                            this.fnChange('editUserInfo', item, event)
                          }
                        />
                      </Form.Item>
                    )}
                          ...
                          {item !== 'key' && (
                            <Form.Item label={item}>
                              <Input
                                ...
                                onChange={event =>
                                  this.fnChange('addData', item, event)
                                }
                              />
                            </Form.Item>
                          )}
