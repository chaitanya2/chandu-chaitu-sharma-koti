...
class Redxflow extends React.Component {
  ...
  componentWillReceiveProps({starwardata}){
      if(starwardata) {
        this.setState({ data: starwardata }); 
      }
  }
  window.alert()
  fnChange = (key, item, e) => {
  ...
  addBackendData = e => {
    ...
    console.log(a);
    this.props.dispatch(addBackendData({ number: a }));
  };
  ...
  render() {
    ...
    var t=JSON.stringify(this.props.starwardata);
    window.alert(t);
    return (
      <div className="ant-table">
        ...
        <div className="App">
          ...
          {/* <button onClick={this.addBackendData0}>saga request</button> */}
          <div>Enter User ID</div>
          <input type="number" onChange={this.addBackendData} />
          {/* <button onClick={this.addBackendData}>Button 1 saga request</button> */}
        </div>
      </div>
    );
  }
